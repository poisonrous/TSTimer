...
const Home = () => {
  ...
  const handlePostPlaylist = async (tracks, totalDuration) => {
    try {
      ...
      const response = await fetch(`${backendUrl}/api/post-playlist`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleClickSave = () => {
    ...
    window.location.href = `${backendUrl}/login`; // Redirigir a la autenticación
  };
  ...
  const handleSavePlaylist = async (accessToken, tracks) => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/save-playlist`, {
        method: 'POST',
        headers: {
